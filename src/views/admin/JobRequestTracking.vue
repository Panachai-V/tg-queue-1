  ...
  <section class="section-full pull-right">
    <div class="container">
      ...
      <div class="section-header mb-4" data-aos="fade-up" data-aos-delay="0">
        <div class="btns mt-0">
          <a href="/admin/job-requests/3" class="btn color-gray h-color-01">
        ...
        <div class="header-wrapper">
          <div class="text-container">
            <h6 class="h3">Job Request Tracking</h6>
          </div>
        </div>
      </div>
      ...
      <div class="section-px section-py-grid" data-aos="fade-up" data-aos-delay="150">
        <div v-if="!dataset.confPickupDate || !dataset.confPickupTime || !dataset.qrCode">
          <form action="/" method="GET" @submit="onScheduleSubmit">
            <div class="grids">
              <div class="grid md-1-3 sm-50 xs-80">
                <FormGroup 
                  type="datepicker" label="วันเข้ารับสินค้า *" placeholder="โปรดระบุ" 
                  wrapperClass="append" :append="true" icon="calendar.svg" 
                  :value="dataset.pickupDate" :required="true" 
                  @input="(event) => dataset.confPickupDate = event"
                />
              </div>
              <div class="grid xl-25 lg-30 md-40 sm-50 xs-80">
                <FormGroupTime
                  label="เวลาเข้ารับสินค้า *" placeholder="โปรดระบุ" 
                  wrapperClass="append" :append="true" icon="calendar.svg" 
                  :value="dataset.pickupDate" :required="true" 
                  @input0="(event) => cpHours = event" 
                  @input1="(event) => cpMinutes = event" 
                />
              </div>
              <div class="grid sm-100">
                <div class="btns mt-0">
                  <Button 
                    type="submit" text="ยืนยันเวลา" 
                    classer="btn-color-01" :append="true" icon="calendar-white.svg" 
                  />
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    onScheduleSubmit(e) {
      e.preventDefault();
      this.dataset.qrCode = '/assets/img/misc/qr-code.jpg';
      this.dataset.confPickupTime = this.cpHours+'.'+this.cpMinutes;
